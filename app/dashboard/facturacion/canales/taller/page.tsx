...
const SEDES: Record<string, string> = { '11': 'Norte', '12': 'Pasoancho', '13': 'Sede 39' }
...
export default function TallerPage() {
  ...
  const [ordenCol,   setOrdenCol]   = useState<'registro'|'cliente'|'sede'|'neto'>('neto')
  const [ordenDir,   setOrdenDir]   = useState<'asc'|'desc'>('desc')

  const cargar = useCallback(async () => {
    setLoading(true); setError('')
    try {
      const [tal, ppto] = await Promise.all([fetchCSV(GID_TALLER), fetchCSV(GID_PRESUPUESTO)])
      setTallerRaw(tal); setPptoRaw(ppto); setUltimaAct(new Date())
  ...
  const presupuesto = useMemo(() => {
    ...
    return pptoRaw.slice(1)
      .filter(r => r[4]?.trim() === 'Taller')
      .reduce((s, r) => s + parseCOP(r[mesIdx]), 0)
  }, [pptoRaw, mes])

  // ── Facturas talleres 11, 12, 13 ─────────────────────────────────────────
  // Taller(0), Refer.(1), Nombre cliente(5), F.cierre(6), Prefijo/num(8), Neto(14), Costo(15)
  const facturas = useMemo((): Factura[] => {
    const mapa: Record<string, Factura> = {}
    tallerRaw.slice(1).forEach(r => {
      const taller = r[0]?.toString().trim()
      if (!['11','12','13'].includes(taller)) return
  ...
  const porSede = useMemo(() => {
    ...
    facturas.forEach(f => {
      if (!m[f.sede]) m[f.sede] = { neto:0, costo:0, facturas:0 }
      m[f.sede].neto     += f.neto
      m[f.sede].costo    += f.costo
      m[f.sede].facturas += 1
    })
  ...
  const facturasFiltradas = useMemo(() => {
    let lista = [...facturas]
    if (filtroSede !== 'todas') lista = lista.filter(f => f.sede === filtroSede)
    if (buscar) {
      const b = buscar.toLowerCase()
      lista = lista.filter(f =>
        f.registro.includes(b) || f.numFactura.toLowerCase().includes(b) ||
        f.cliente.toLowerCase().includes(b) || f.sede.toLowerCase().includes(b)
      )
    }
    lista.sort((a,b) => {
    ...
    return lista
  }, [facturas, filtroSede, buscar, ordenCol, ordenDir])
    ...
    <div className="p-6 max-w-[1400px] mx-auto space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-brand-subtle">Talleres 11 (Norte) · 12 (Pasoancho) · 13 (Sede 39)</p>
        </div>
            ...
            <Panel key={sede}>
              ...
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  <p className="text-xs text-brand-subtle font-mono">{datos.facturas} facturas</p>
                </div>
                {filtroSede===sede && (
                  <span className="text-xs px-2 py-0.5 rounded-full bg-brand-teal/20 text-brand-teal font-mono">Activo</span>
                )}
              </div>
      ...
      <Panel>
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-2">
            ...
            {filtroSede !== 'todas' && (
              <span className="text-xs px-2 py-0.5 rounded-full bg-brand-teal/20 text-brand-teal font-mono">
                {filtroSede} ✕
              </span>
            )}
          </div>
          <div className="flex gap-2 flex-wrap">
            <select value={filtroSede} onChange={e=>setFiltroSede(e.target.value)}
              className="bg-brand-bg border border-brand-border rounded-lg px-3 py-1.5 text-sm text-brand-text font-mono focus:outline-none focus:border-brand-teal">
              <option value="todas">Todas las sedes</option>
              <option value="Norte">Norte (11)</option>
              <option value="Pasoancho">Pasoancho (12)</option>
              <option value="Sede 39">Sede 39 (13)</option>
            </select>
            <input type="text" placeholder="Buscar factura o cliente..."
              value={buscar} onChange={e=>setBuscar(e.target.value)}
              className="bg-brand-bg border border-brand-border rounded-lg px-3 py-1.5 text-sm text-brand-text font-mono focus:outline-none focus:border-brand-teal w-56"/>
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-brand-border">
                ...
                <th className={thCls('sede')} onClick={()=>ordenar('sede')}>Sede {icono('sede')}</th>
              ...
              {facturasFiltradas.map(f => {
                ...
                return (
                  <tr key={f.registro}
                    ...
                    <td className="py-3 pr-4 text-brand-text font-medium max-w-[220px] truncate">{f.cliente}</td>
            ...
            <tfoot>
              <tr className="border-t-2 border-brand-border">
                ...
                <td colSpan={3} className="pt-3 font-mono text-xs text-brand-subtle">
      ...
      <p className="text-xs text-brand-subtle font-mono text-center pb-4">
        Talleres 11, 12, 13 · Datos desde Google Sheets · Actualización cada 6 horas
      </p>
    </div>
  )
}
