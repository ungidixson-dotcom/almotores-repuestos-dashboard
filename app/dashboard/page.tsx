...
import {
  ...
  LayoutDashboard, Car, Printer, ChevronRight,
} from 'lucide-react'
...
const NAV_ITEMS = [
  { id: 'subastas',   label: 'Subastas',    icon: LayoutDashboard },
...
export default function Dashboard() {
  ...
  const [activeNav,            setActiveNav]            = useState<NavId>('subastas')
  ...
  return (
    <>
      ...
      <div className="flex min-h-screen bg-brand-bg">
        ...
        ══════════════════════════════════════════════════════════════════ */}
        <aside
          className={`no-print fixed top-0 left-0 h-screen ${sidebarW} bg-brand-surface border-r border-brand-border flex flex-col z-20 transition-all duration-200`}
        >
          {/* Logo / marca */}
            ...
            {NAV_ITEMS.map(item=>{
              ...
              return (
                ...
                >
                  {/* Indicador activo */}
                  {active && (
                    <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-brand-teal rounded-r"/>
                  )}
          ...
          {/* Footer del sidebar */}
          <div className="border-t border-brand-border py-3 space-y-1">
            {/* Botón imprimir */}
            <button
              onClick={handlePrint}
            ...
            >
              <ChevronRight size={14} className={`flex-shrink-0 transition-transform duration-200 ${sidebarCollapsed?'':'rotate-180'}`}/>
        ...
        <div className={`flex-1 flex flex-col min-h-screen transition-all duration-200 ${sidebarCollapsed ? 'ml-16' : 'ml-56'} print-area`}>
          ...
          <header className="no-print sticky top-0 z-10 border-b border-brand-border bg-brand-surface/60 backdrop-blur-sm px-6 py-2.5 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="font-mono text-[10px] text-brand-muted uppercase tracking-widest">
                {NAV_ITEMS.find(n=>n.id===activeNav)?.label} · Ene–Jul 2026
              </span>
            </div>
            <div className="flex items-center gap-3">
              ...
              {/* Botón PDF en topbar (visible en pantalla, se oculta al imprimir) */}
              <button
                onClick={handlePrint}
          ...
          <main className="flex-1 p-6">
            ...
            {/* Título del informe — visible en impresión */}
            <div className="mb-6">
              <h1 className="font-title text-2xl font-bold text-brand-text">
                Torre de Control · Subastas
              </h1>
              <p className="text-brand-subtle text-sm mt-1">
                Análisis en tiempo real — Enero a Julio 2026
              </p>
            </div>
